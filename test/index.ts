...
Deno.test(`Uwufy words & check for inconsistency`, () => {
  const transformedWords = OriginalWords.map((word) => uwuifier.uwuifyWords(word));
  assertEquals(transformedWords, uwuifiedWords);
});

Deno.test('Uwufy sentences & check for inconsistency', () => {
  const transformedSentences = originalSentences.map((sentence) => uwuifier.uwuifySentence(sentence));
